...
interface AssetPickerSheetProps {
  ...
  /** When true, renders as a plain inline panel (no bottom-sheet chrome, no backdrop) */
  inline?: boolean
}

export function AssetPickerSheet({ open, onClose, selectedIds, onToggle, inline = false }: AssetPickerSheetProps) {
  ...
  // Load whenever search/filter changes — not gated on `open` so desktop sidebar
  // always has data, and search works without needing to re-open the sheet
  useEffect(() => {
    setPage(1)
    setDocs([])
  ...
  // Inline mode: plain panel without bottom-sheet chrome
  if (inline) {
    return (
        ...
        <div className="px-3 pt-2 pb-2 flex-shrink-0 space-y-2">
            ...
            {['', 'image', 'video'].map((t) => (
              <button
                ...
                className={cn(
                  ...
                  typeFilter === t ? 'bg-gallery-gold/15 text-gallery-gold' : 'bg-on-surface/5 text-on-surface/40 hover:text-on-surface/60',
                )}
          ...
          {totalDocs > 0 && (
            <p className="text-[10px] text-on-surface/30">
              {docs.length} of {totalDocs} · {selectedIds.size} selected
            </p>
          )}
        </div>
        <div className="flex-1 overflow-y-auto px-3 pb-4">
          {loading && docs.length === 0 ? (
            ...
            <>
                ...
                {docs.map((media) => {
                  ...
                  return (
                    <button
                      ...
                      aria-label={`${selected ? 'Remove' : 'Add'} ${media.title ?? media.filename}`}
                      className={cn(
                        'relative aspect-square rounded-xl overflow-hidden transition-all duration-200',
                        selected ? 'ring-2 ring-gallery-gold ring-offset-1 scale-95' : 'hover:scale-[0.97]',
                      )}
                    >
                      {previewUrl ? (
                        <img src={previewUrl} alt={media.alt ?? ''} className="w-full h-full object-cover" loading="lazy" />
                      ) : (
              ...
              {hasNext && (
                <button onClick={loadMore} disabled={loading} className="w-full mt-3 py-2 text-[11px] text-gallery-gold hover:underline disabled:opacity-50">
    ...
    <>
      ...
      <div
        ...
        className={cn(
          ...
          'shadow-[0px_-24px_64px_rgba(0,0,0,0.18)] lg:shadow-none',
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 flex-shrink-0">
          ...
          <button
            ...
            className="w-7 h-7 flex items-center justify-center rounded-full bg-on-surface/5 text-on-surface/40 hover:text-primary active:scale-90 transition-all"
          >
            <X size={14} />
          </button>
        </div>
        ...
        {/* Count indicator */}
        {totalDocs > 0 && (
          <p className="px-3 pb-1 text-[10px] text-on-surface/30 flex-shrink-0">
            Showing {docs.length} of {totalDocs} · {selectedIds.size} selected
          </p>
        )}
